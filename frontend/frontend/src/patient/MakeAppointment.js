...
function MakeAppointmentPage() {
  ...
  const showSuccesHandler = (event) =>{
    ...
    const data ={
      ...
      is_confirmed: false,
      patient: `http://127.0.0.1:8000/auth/users/${localStorage.getItem("owner")}`,
        ...
        <div className={styles.container}>
          ...
          <div className={styles.body}>
            <div className={styles.left}>
                ...
                <select className={styles.doctors}  value={selectedDoc} onChange={handleDoctorChange} id="doctors" >
                <option value="">Wybierz opcję</option>
                  {crewFetch.map((doctor)=>(
                    <option  value={doctor.id} key={doctor.id}>{doctor.first_name +' ' + doctor.last_name}</option>
                  ))}
                  
                </select>
                <h2 className={styles.h2_}>Opis dolegliwości</h2>
                <textarea id="issues" name="issues" rows="10" cols="80" onChange={handleDescription}></textarea>
            </div>
            <div className={styles.right}>
              ...
              <div className={styles.date}>
                ...
                <p>Wybrana data: {selectedDate.toDateString()}  {selectedTime}</p>
              </div>
              {succesIsShown && <SuccessSubmit onHideCart={hideSuccesHandler} dataSuc={succData} timeSuc={selectedTime}/>}
